...
func test_create_battler():
	...
	# arrange
	var scene = load("res://Combat/Character.tscn")
	var linda = get_linda()

	# act
	var battler = combat_init.create_battler(scene, linda, Vector2(2, 2))
...
func test_create_battlers():
	...
	var character_scene = load("res://Combat/Character.tscn")
	var combat = Combat.new()
	add_child(combat)

	# act
	var characters = combat_init.create_battlers(character_scene, combat, character_stats)
